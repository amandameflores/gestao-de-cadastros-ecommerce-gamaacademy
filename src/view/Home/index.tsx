    ...
    <Container fluid>
      <div>
        ...
        <div>
          ...
          <Button color="red" disabled={disabled} onClick={handleButton}>Limpar carrinho</Button>
        </div>
        <div>
          {
            cart.map( (prod) => (
              <div key={prod.id}>
                <img src={`${process.env.PUBLIC_URL}/img/${prod.photo}`} alt="iphone" width="200" height="auto" />
            <h4>{prod.name}</h4>
            <span>{prod.description}</span>
            <h6>{prod.price}</h6>
            <Button color="red" onClick={ () => removeCart(prod.id)}> Remover do carrinho</Button>
              </div>
            ))
          }
          </div> 
      ...
      <section>
        { products.map( (prod) => (
          <div key={prod.id}>
            ...
            <span>{prod.description}</span>
            <h6>{prod.price}</h6>
            <Button color="blue" onClick={ () => addCart(prod.id)}> Adicionar ao carrinho</Button>
          </div>
        ))}
      </section>
    </Container>
